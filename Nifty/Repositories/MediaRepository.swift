...
protocol MediaFetcheable {
    func fetchImageData(from url: URL) -> AnyPublisher<Media, Error>
}
...
final class MediaRepository: MediaFetcheable {
    ...
    // Do persistence
    // check if file exitsts FileManager.default.fileExists(atPath: documentsDirectoryURL.appendingPathComponent(videoURL.lastPathComponent).path)
    func fetchImageData(from url: URL) -> AnyPublisher<Media, Error> {
